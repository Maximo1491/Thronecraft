...
namespace octet {
  class mesh_builder {
    ...
    void add_front_face(float size, voxel::VoxelType vt = voxel::voxelType_default) {
      unsigned short cur_vertex = (unsigned short)vertices.size();
      if(vt == voxel::VoxelType::voxelType_grass)
      {
        add_vertex(vec4(-size, -size, size, 1), vec4(0, 0, 1, 0), 0, 0, vec4(0, 1, 0, 1));
        add_vertex(vec4(-size,  size, size, 1), vec4(0, 0, 1, 0), 0, 1, vec4(0, 1, 0, 1));
        add_vertex(vec4( size,  size, size, 1), vec4(0, 0, 1, 0), 1, 1, vec4(0, 1, 0, 1));
        add_vertex(vec4( size, -size, size, 1), vec4(0, 0, 1, 0), 1, 0, vec4(0, 1, 0, 1));
      }
      if(vt == voxel::VoxelType::voxelType_dirt)
      {
  ...
  public:
    ...
    // as in glutSolidCube
    void add_cube(float size, voxel::VoxelType vt = voxel::VoxelType::voxelType_default) {
      add_front_face(size, vt);
      matrix.rotateY90();
      add_front_face(size, vt);
      matrix.rotateY90();
      add_front_face(size, vt);
